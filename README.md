...
This repository's own logbook currently shows zero token usage for every milestone. The real reason: this project was built by subagents dispatched from a sibling development session, so the Claude Code transcripts that cover the actual work are filed under that sibling session's own project path, not this one. This project's exact-match project isolation (see `AGENTS.md`) correctly declines to guess or reach into another project's transcripts, so it reports zero and says why, rather than quietly borrowing numbers that were never really this project's own.
