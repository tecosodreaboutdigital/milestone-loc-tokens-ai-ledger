...
**Do not delete `data.json` to "unfreeze" costs.** Besides discarding the audit trail, it discards every frozen token count, and a transcript that gave those tokens may no longer exist (a subagent transcript filed under another project, a cleaned-up session), so they cannot be regenerated. `--reprice` exists so that nobody has to, and `--enrich` (next section) is the supported route when it is the token schema, not the price, that changed.

## Enriching milestones recorded before a schema change

A milestone's `tokens` gained fields over time: `cache_creation_1h` and `by_model` (see "Multiple models and cache TTLs"). A milestone frozen before that has only the four original counters, so it is priced with the one configured series and every cache write at the 5-minute rate, and `--reprice` cannot fix it: it recomputes from the frozen tokens, and those lack the split. `--enrich` is the supported way to bring such a milestone up to date, once.

**The practice behind it: freeze the finest-grained fact you price, and derive the price later.** A recorded cost is a derived number; the tokens, per model and per cache TTL, are the fact. Frozen at that grain, any later price correction is a `--reprice` and needs no transcript. A transcript is needed again only when the schema itself gains a dimension, and only for milestones frozen before it. A usage row in a transcript already carries more than the engine records (`speed`, `service_tier`, `inference_geo`, `server_tool_use`); whether any of them changes a price has not been verified here, so read this as the reason a migration could be needed again, not as a claim that it will be.

1. Run `python engine/generate_metrics.py --repo <path> --enrich --dry-run`, with `--linked-project <other>` exactly as for a normal run (after confirming it, as above). It reads git history and the transcripts, prints what it would do and writes nothing.
2. Read the report. Commit `data.json` first, so the state before is in git.
3. Run it again without `--dry-run`.

**The guard.** A milestone is enriched only if the four original counters (`input`, `output`, `cache_read`, `cache_creation`) re-derived from the transcripts equal the frozen ones exactly. Otherwise it is refused and left byte for byte as it was, and the report shows the frozen and the re-derived value of each counter that differs. A refusal means the transcripts do not prove the split: a transcript was deleted, a `--linked-project` was not repeated, or the history was rewritten. Fix the cause and run again; never make a milestone pass by editing its frozen counters, and never assume a split for it.

**What it changes**, for each enriched milestone only: `tokens` gains `cache_creation_1h` and `by_model`; `cost_recorded` and `unpriced` are recomputed per model and per TTL against the ledger as it is now (honouring `price_mode`); one record is appended to its `repricings` list with the cost (and unpriced list) it had before and `"reason": "enrich"`, even when the cost did not change, because the tokens did; `data.json` gains `last_enriched_at`. `generated_at`, words, lines, subject, note and the four counters are never touched. The dashboard and thumbnails are re-rendered. A second run finds nothing left to do. The exit code is 3 whenever at least one milestone was refused, even if others were written, so a script notices.

**The cost can go either way.** Per-model pricing prices a cheaper model lower and a dearer one higher, 1-hour writes at their own price, and a model with no series in the ledger stays unpriced (the milestone then shows "(partial)"), never priced at another model's rate. The dry-run report splits the change into 1-hour cache writes and model mix, and lists what would become partial. If it warns that some recorded costs already differ from the current ledger, run `--reprice` first so the enrichment shows only the split.

**There is a deadline.** Claude Code deletes transcripts, subagent transcripts included, once their file modification time is older than `cleanupPeriodDays`: 30 days by default, 1 at the minimum. After that, the milestones they covered can no longer be enriched and the guard refuses them for good. The docs also say the entry format is internal to Claude Code and changes between versions, so a Claude Code release can break the reader. Sources read on 20 September 2026: [claude-directory](https://code.claude.com/docs/en/claude-directory) ("Cleaned up automatically") and [sessions](https://code.claude.com/docs/en/sessions.md) ("Where transcripts are stored"); re-read them before relying on the numbers. So enrich early, or keep the option open by copying the project's transcript folder aside or raising `cleanupPeriodDays` (a `SessionEnd` hook can also archive a transcript, same page).
...
- Never recalculate a milestone's recorded cost after the fact, even when the ledger gains a newer price. The two exceptions are `--reprice`, the explicit, audited correction of a price that was **wrong** (see "Correcting a price that was wrong"), never a route for a normal price change, and `--enrich`, the one-time migration of a milestone frozen before the token schema gained the per-model and per-TTL split, only where the transcripts prove it (see "Enriching milestones recorded before a schema change").
...
- Never run `--enrich` for real before reading its `--dry-run` report, never assume a cache TTL or a model for a milestone it refused, and never edit a frozen counter to make one pass.
